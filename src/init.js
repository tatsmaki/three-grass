import * as entity_manager from "./base/entity-manager.js";
import * as entity from "./base/entity.js";

import { threejs_component } from "./base/threejs-component.js";

import { sky, renderSky } from "./game/render/sky.js";
import { light } from "./base/render/hemi-light.js";
import { createGrass, renderGrass } from "./base/render/grass-component.js";
import { createTerrain } from "./base/render/terrain-component.js";

export const init = (canvas) => {
  const entityManager = entity_manager.EntityManager.Init();

  const threejs = new entity.Entity("threejs");
  threejs.AddComponent(new threejs_component.ThreeJSController(canvas));
  threejs.Init();

  const camera = threejs.GetComponent("ThreeJSController").Camera;
  const controller = threejs.GetComponent("ThreeJSController");
  const renderer = controller.renderer;
  const scene = controller.scene;

  scene.add(sky, light);

  camera.position.set(10, 1.95, 0);
  // camera.position.set(0, 20, 0);
  camera.lookAt(0, 4, 10);

  const terrain = createTerrain();
  const grass = createGrass();

  scene.add(terrain, grass);

  document.getElementById("canvas").classList.add("ready");

  let prevTime = 0;

  renderer.setAnimationLoop((time) => {
    const dt = (time - prevTime) * 0.00025;
    prevTime = time;

    entityManager.Update(dt);

    renderGrass(camera, time * 0.00025);
    renderSky(dt);
    controller.Render(dt);
  });

  return controller;
};
